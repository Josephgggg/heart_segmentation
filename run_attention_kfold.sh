#!/usr/bin/env bash
# Launches the 5-fold attention-UNet runs (--arch attention_unet). Sibling of
...
#
# Read the comparison as "MONAI AttentionUnet vs milesial UNet", NOT as "do
# attention gates help": MONAI's network also swaps maxpool downsampling for
# strided convs and concatenation for a gated merge conv. See
# unet/attention_unet.py's module docstring.
#
...
#
# Usage:
#   bash run_attention_kfold.sh          # uses every GPU nvidia-smi reports
#   bash run_attention_kfold.sh 0 2 3    # or pin it to specific GPU indices
#
...
if [ -z "${KFOLD_DETACHED:-}" ]; then
    LOG="$(pwd)/run_attention_kfold.out"
    echo "Detaching so this survives a dropped connection."
    echo "  progress (this script):  tail -f $LOG"
    echo "  progress (a given run):  tail -f logs_both_attn_kfold*.log"
...
if [ "$N_GPUS" -eq 0 ]; then
    ...
    echo "  bash run_attention_kfold.sh 0 1" >&2
    exit 1
fi
