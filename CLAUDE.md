...
Two architectures are selectable, via `--arch` / `build_model(arch, ...)` in `train.py`, and the choice is recorded as `arch` in `run_config.json` (absent/`None` in every run predating the flag, all of which are `unet`):

- `unet` (default) — the above, used by every run so far.
- `unet_attn_gates` — **the clean attention ablation.** `unet/attention_gate_unet.py` is exactly the above UNet with MONAI's `AttentionBlock` (Oktay 2018) gating each skip connection before concatenation, and nothing else changed: same `DoubleConv`/`Down`/`OutConv`, same widths, same maxpool, same `bilinear` handling, same `Up` padding. Verified structurally — every `UNet` parameter name and shape is preserved and the gates add 84 tensors (+352k params, +1.13%). A difference against the `unet` control is attributable to the gates alone. `run_attn_gates_kfold.sh` trains the 5 folds. Use this arm to ask "do attention gates help".

  Result of the `unet_attn_gates` 5-fold run (2026-09-06, `both_attngate_kfold{0..4}`, pooled validation, 42 patients, paired Wilcoxon vs the `both_data_aug_dice_ce_kfold*` control): macro Dice 0.8989 vs 0.8985, median paired difference **+0.0007, p = 0.74**; HD95 p = 0.89, ASSD p = 0.20; no per-chamber test comes close (every `p_holm` = 1.0). **A clean null — attention gates change nothing measurable on this task.** Read together with the `attention_unet` result below, the whole-network arm's small but reliable deficit (−0.0021, p = 0.0065) is therefore attributable to MONAI's *other* changes (strided-conv downsampling, conv-based skip fusion), not to attention. Per-fold deltas were split in sign (−0.0011 / +0.0009 / −0.0011 / +0.0025 / +0.0024), the signature of no effect rather than a small one.
- `attention_unet` — MONAI's `AttentionUnet` (Oktay 2018), wrapped by `unet/attention_unet.py` to expose the `n_channels`/`n_classes`/`bilinear` attributes the rest of the codebase reads off a model. **This is a whole-network swap, not attention gates added to `UNet`**: MONAI also downsamples with stride-2 convs instead of maxpool, always upsamples with transposed convs (`--bilinear` is rejected, not ignored), and fuses gated skip + decoder feature with a conv instead of concatenating into a `DoubleConv`. Widths and depth are matched to `UNet`'s 64/128/256/512/1024 and the parameter counts land close (31.74 M vs 31.04 M at `n_channels=2, n_classes=6`), but a difference between the two arms is "this architecture vs that architecture", not evidence about attention gates specifically. `run_attention_kfold.sh` trains the 5 folds; its control arm is the already-finished `both_data_aug_dice_ce_kfold{0..4}`, whose config it matches exactly apart from `--arch`. Use this arm to ask "how does the standard reference implementation compare", not "do attention gates help" — that is `unet_attn_gates`.

Result of the `attention_unet` 5-fold run (2026-09-05, `both_attn_kfold{0..4}`, pooled validation, 42 patients, paired Wilcoxon vs the `both_data_aug_dice_ce_kfold*` control): macro Dice 0.8948 vs 0.8985, median paired difference −0.0021, p = 0.0065 on the pre-specified primary endpoint; HD95 (+0.08 mm, p = 0.19) and ASSD (+0.02 mm, p = 0.059) unchanged; no per-chamber test survives Holm. Statistically reliable, practically negligible, and it is an architecture comparison — not evidence about attention gates.

MONAI is an extra dependency of that arm only (hence the lazy import in `build_model`). **Install anything into `unet_env` with `pip install --no-deps`.** On 2026-09-05 an interrupted pip run left torch's `dist-info` renamed, so pip could not see torch as installed at all: a plain `pip install monai` then resolved torch as missing, downloaded a CUDA-13 torch plus the `nvidia-*` CUDA-13 wheels over the top of it, and those overwrote the shared `nvidia/cudnn/lib` and `nvidia/nccl/lib` files that torch 2.5.1+cu118 needs — breaking every GPU run in the venv. That was repaired; as of 2026-09-10 `pip show torch` correctly reports `2.5.1+cu118`, so the renamed-directory trap specifically is gone. `--no-deps` stays the rule anyway, because what did the damage was a resolver deciding it was entitled to replace torch, and that decision is one dependency bump away at any time.

## nnU-Net benchmark arm (a separate pipeline, NOT an `--arch`)

nnU-Net v2 is compared against `both_data_aug_dice_ce_kfold{0..4}`, but it is deliberately **not** wired into `build_model`, and it should not be. nnU-Net is a framework, not a model: it owns preprocessing, resampling, patch-size and architecture planning, training and inference, and its authors' central claim is that *those* are what win rather than the network. Lifting its `PlainConvUNet` into `train.py` would give a generic UNet and none of nnU-Net. The general rule this follows: **port the recipe into this repo when the recipe is parameters (optimizer, LR, schedule, epochs, loss); run the upstream pipeline and bridge only the predictions when the recipe *is* a data pipeline.**

The invariant that makes the comparison possible is that **the patient split and the metrics stay ours**. Three scripts implement that:

- `export_nnunet_dataset.py` (run in `unet_env`) — writes `nnUNet_raw/Dataset501_CardiacBoth` from `data/preprocessed_cache/*_both_{img,mask}.npy`, i.e. from exactly the arrays `VolumeMRIDataset` built and every `both_*` run trained on. Water is channel `_0000`, fat `_0001`, labels 0–5 with EAT as class 5. Four things it is careful about: intensities stay **raw** (nnU-Net z-scores per image itself, and that is part of the method — note intensity scale is not even consistent across this cohort, some volumes are 0–255 and others run past 1700); resolution stays **native 432²**, not `--scale 0.5`; affines are built per patient from `spacing_for()` at `scale=1.0`, because **the source NIfTI headers are junk** (`get_zooms()` reports ~`(696759, 696759, 1250000)`), which is why this repo reads spacing from the DICOM; and the 8 held-out **test patients go to `imagesTs`**, which nnU-Net never preprocesses for training, so leakage is structurally impossible rather than merely configured.
- `make_nnunet_splits.py` — writes `splits_final.json` by copying fold membership verbatim out of the control runs' `run_config.json`. nnU-Net invents its own 5-fold split unless this file exists, and an invented split means `compare_runs` has nothing to pair. Run it *after* `nnUNetv2_plan_and_preprocess` and *before* `nnUNetv2_train`; `run_nnunet_kfold.sh` re-verifies it (5 folds partitioning 42 patients, no train/val overlap) rather than trusting it.
- `score_nnunet_predictions.py` (run in `unet_env`) — turns nnU-Net's validation NIfTIs into the standard `val_metrics_per_patient.csv`. It re-implements no metric: `metrics.rows_for_volume` is already model-free, so the only new code is assembling the GT volume (by stacking `ds[i]['mask']` in slice order, so the GT is preprocessed by the same path that scored every other arm) and aligning the prediction to it. `--self-check` validates that assembly against the raw cache with no GPU and no predictions.

**`nnunet_env`, not `unet_env`.** nnunetv2 has 22 runtime dependencies and genuinely needs them, so `--no-deps` is unavailable; installing that into `unet_env` would put every finished experiment's reproducibility at the mercy of a dependency bump. It lives at `/hpc/jgeo610/Virtual_ENV/nnunet_env` and `unet_env` is never touched. Install torch **first**, pinned to cu118, or `pip install nnunetv2` pulls a default-CUDA torch.

Two things to know before reading any nnU-Net number:

- **It is 3d_fullres against a 2D control**, so part of any difference is 3D-vs-2D rather than nnU-Net-vs-UNet — the control never sees through-plane context. A reporting caveat, not a bug.
- **The scoring grid is an open decision.** The control was trained and scored at 216² (`img_scale=0.5`, in-plane spacing 1.3935 mm); nnU-Net works at native 432² (0.6968 mm). `score_nnunet_predictions.py --grid` therefore has **no default**: `matched` downsamples nnU-Net to 216² and pairs directly against the existing CSVs but handicaps it at scoring time, while `native` scores at full resolution but requires regenerating the control's CSVs by upsampling its predictions first.

**Multiplicity across arms.** `compare_runs` leaves the macro row uncorrected as *a single* pre-specified primary endpoint, which is valid for one arm against one control. With `unet_attn_gates`, `attention_unet` and now nnU-Net all tested against the same control, that is three primary tests — freeze the arm list before running and Holm-correct across arms as well.
