...
MONAI is an extra dependency of that arm only (hence the lazy import in `build_model`). **Install anything into this venv with `pip install --no-deps`.** `torch` there is a `2.5.1+cu118` build whose `dist-info` was left renamed by an interrupted pip run, so pip does not see torch as installed at all: a plain `pip install monai` resolves torch as missing, downloads a CUDA-13 torch plus the `nvidia-*` CUDA-13 wheels over the top of it, and those overwrite the shared `nvidia/cudnn/lib` and `nvidia/nccl/lib` files that torch 2.5.1+cu118 needs — breaking every GPU run in the venv. (This happened on 2026-09-05 and was repaired.)
