#!/usr/bin/env bash
# Launches the 5-fold 2.5D runs (context_slices = 1, 2, 3) used by
...
# context_slices instead of augment on/off or loss. The 2D arm the notebook
# compares all three against reuses both_data_aug_dice_ce_kfold{0..4} (already
# finished, phase=both, augment=True, loss=dice_ce, split_seed=0, seed=fold --
# exactly this script's config apart from context_slices), so there is
# nothing to train for it.
#
...
# GPUs. See the notebook's intro cell. Pass a single GPU index to force
# strictly sequential execution (one job runs to completion before the next
# starts) across every context_slices value and fold.
#
# Usage:
...
if [ -z "${KFOLD_DETACHED:-}" ]; then
    ...
    echo "  progress (a given run):  tail -f logs_both_ctx_cs*_kfold*.log"
...
PHASE=both
LOSS=dice_ce
CONTEXT_SLICES_LIST=(1 2 3)   # the axis this script trains -- must match the notebook's
                              # CONTEXT_SLICES_FOR['cs1'/'cs2'/'cs3']
EPOCHS=40
BATCH_SIZE=8
...
# every 2.5D arm means, at a given fold, only context_slices differs between
# them. Outer loop is context_slices, inner is fold -- same order as
# run_both_phase_loss_ablation_kfold.sh's arm/fold loop, so all 5 folds of one
# context_slices value finish before the next value starts.
for cs in "${CONTEXT_SLICES_LIST[@]}"; do
    for fold in $(seq 0 $((K_FOLDS - 1))); do
        run_name="${RUN_PREFIX}_cs${cs}_kfold${fold}"
...
wait "${pids[@]}"
echo "All done. Check logs_${PHASE}_ctx_cs*_kfold*.log for any that failed."
